...
export default function Dashboard() {
  ...
  return (
    <>
      <PageHead eyebrow="01 · Сводка" title="Состояние реестра">
        <Link className="btn" href="/documents">
          <span className="row" style={{ gap: 8 }}><Glyph.docs size={15} /> Загрузить прайс</span>
        </Link>
      </PageHead>
      ...
      {data && (
        <>
          ...
          <div className="panel pad">
            ...
            <div className="row" style={{ marginTop: 16, gap: 22, fontSize: 13 }}>
  ...
  return (
    <div>
      <div className="between" style={{ marginBottom: 8 }}>
        ...
        <span className="num" style={{ fontSize: 22 }}>{n.auto_match_pct}% авто</span>
      </div>
      <div style={{ display: "flex", height: 16, border: "1.5px solid var(--ink)" }}>
...
function Legend({ color, label, v }: { color: string; label: string; v: number }) {
  return (
    <span className="row" style={{ gap: 7 }}>
      <span style={{ width: 11, height: 11, background: color, border: "1px solid var(--ink)" }} />
      <span>{label}</span>
